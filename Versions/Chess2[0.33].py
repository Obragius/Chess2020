			...
			if ActiveMoves != None:
				...
				while Moves < len(ActiveMoves):
					...
					if Stop == 1:
						...
						GreyAttribute = 0
						# Getting the piece from board
						Row = int(str(ActivePiece)[0])-1
						Column = int(str(ActivePiece)[1])-1
						Piece = Board().GetPieceFromBoard(Row,Column)
						# Getting the coresponding image to that piece
						ImageID = ImageToLoadDict[Piece]
						self.button[6][6].SetBitmap(ImageToLoad[ImageID][GreyAttribute])
						break
					Moves +=1
