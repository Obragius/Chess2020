			...
			for j in range(8):
				...
				if Column+DiagDirection[i][1] > 0 and Column+DiagDirection[i][1] < 9 and Row+DiagDirection[i][0] > 0 and Row+DiagDirection[i][0] < 9:
					...
					while Moves < len(BoundMoves):
						...
						NextPositionY = str(int(ActualPosition[0])+DiagDirection[i][0])
						NextPositionX = str(int(ActualPosition[1])+DiagDirection[i][1])
						...
						if NextPosition == TempPosition:
							...
							if CheckPos == "  ":
								...
								for k in range(8):
									...
									NextPositionYK = str(int(ActualPosition[0])+DiagDirectionK[i][0])
									NextPositionXK = str(int(ActualPosition[1])+DiagDirectionK[i][1])
									NextPositionK = NextPositionYK + NextPositionXK
									if NextPositionK == TempPosition:
										BoundMoves.remove(BoundMoves[Moves])
						else:
							Moves +=1
