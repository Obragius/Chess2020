...
class Logic():
	...
	def DirectionDistanceCollision(self, BoundMoves, Row, Column):
		...
		# Setting temporary values to count against for the row
		Moves = 0
		# Highest Distance [0]=Row, [1]=Column
		HighestDistance = [0,0]
		# Highest Position [0]=Row, [1]=Column
		HighestPos = ["",""] 
		# Lowest Distance [0]=Row, [1]=Column
		LowestDistance = [0,0]
		# Lowest Position [0]=Row, [1]=Column
		LowestPos = ["",""]
		# Temporary value [0]=Row, [1]=Column
		Temp = [0,0]
		# Temp Distance [0]=Row, [1]=Column
		TempDistance = [0,0]
		for i in range(2):
			while Moves < len(BoundMoves):
				...
				Temp[i] = int(str(TempPosition[i]))
				TempDistance[i] = Temp[i] - Row
				if TempDistance[i] > 0:
					if TempDistance[i] > HighestDistance[i]:
						HighestDistance[i] = TempDistance[i]
						HighestPos[i] = str(BoundMoves[Moves])
						Moves +=1
					else:
						Moves +=1
				elif TempDistance[i] < 0:
					if TempDistance[i] < LowestDistance[i]:
						LowestDistance[i] = TempDistance[i]
						LowestPos[i] = str(BoundMoves[Moves])
						Moves +=1
					else:
						Moves +=1
				else:
			...
			# Checking [0]=Row, [1]=Column
			Checking = [["",""],["",""]]
			# Starting [0]=Row, [1]=Column
			Starting = [[0,0],[0,0]]
			Direction = [+1,-1]
			Checking[i][0] = Axis[i] + HighestDistance[i]
			Starting[i][0] = Axis[i]+1
			Checking[i][1] = Axis[i] + LowestDistance[i]
			Starting[i][1] = Axis[i]-1
			# TempPosition work around [0]=Column, [1]=Row
			TempPositionNew = [TempPosition[1],TempPosition[0]]
			# AxisNew [0]=Column, [1]=Row
			AxisNew = [Column,Row]
			# Setting up the loop which will do the math
			for j in range(2):
					while Starting[i][j] != Checking[i][j]:
						if Starting[i][j]-1 == 8 or Starting[i][j] == -1:
							break
						BoardSearch = [Board().BoardConfig[Starting[0][j]-1][Column-1],Board().BoardConfig[Row-1][Starting[1][j]-1]]
						CheckPos = BoardSearch[i]
						if CheckPos == "  ":
							Starting[i][j] += int(Direction[j])
						else:
							...
							while Moves < len(BoundMoves):
								...
								TempPosition = str(BoundMoves[Moves])
								if int(TempPositionNew[i]) == AxisNew[i]:
									if j == 0:
										if int(TempPosition[i]) > Starting[i][j]:
