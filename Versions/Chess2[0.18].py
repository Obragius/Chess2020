...
class Logic():
	...
	def DirectionDistanceCollision(self, BoundMoves, Row, Column):
		# Checking for Straight Colision
			# Vertical collision
			# Setting temporary values to count against
			Moves = 0
			...
			# For highest value loop
			# We need to check each space going up and cancel all spaces which are not empty
			CheckingRow = Row + HighestRowDistance
			StartingRow = Row+1
			while StartingRow != CheckingRow:
				CheckPos = Board().BoardConfig[StartingRow-1][Column-1]
				if CheckPos == "  ":
					StartingRow +=1
				else:
					...
					while Moves < len(BoundMoves):
						...
						if int(TempPosition[1]) == Column:
							if int(TempPosition[0]) > StartingRow:
								BoundMoves.remove(BoundMoves[Moves])
							else:
								Moves +=1
						else:
							Moves +=1
					StartingRow = CheckingRow
			# For lowest value loop
			CheckingRow = Row + LowestRowDistance
			StartingRow = Row-1
			while StartingRow != CheckingRow:
				CheckPos = Board().BoardConfig[StartingRow-1][Column-1]
				if CheckPos == "  ":
