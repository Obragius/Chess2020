...
class LoadingImages():
	def initilize(self):
		...
		imageWhite_Knight ,imageWhite_Pawn ,ImageToLoad ,ImageToLoadDict, GreyAttribute)
...
class MyPanel(wx.Panel):
	...
	def __init__(self, parent):
		...
		ImageToLoad ,ImageToLoadDict, GreyAttribute) = LoadingImages().initilize()
		...
		if attackCheck == False:
			...
			if Match == True:
				...
				if PossibleMoves != None:
					...
					Moves = 0
					while Moves < len(PossibleMoves):
						Current = PossibleMoves[Moves]
						GreenRow = int(str(Current)[0]) - 1
						GreenColumn = int(str(Current)[0]) - 1
						self.button[GreenRow][GreenColumn].SetBitmap(ImageToLoad[ImageID][GreyAttribute[Row-1][Column-1]])
