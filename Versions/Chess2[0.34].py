...
class Frame(wx.Frame):
	def __init__ (self, parent, title):
		super(Frame, self).__init__(parent, title=title, size = (900, 900), pos=(600,90))
...
class MyPanel(wx.Panel):
	...
	def __init__(self, parent):
			...
			for j in range(8):
				if (i % 2) == 0:
					if (j % 2) == 0:
						...
						self.hbox[i].Add(self.button[i][j], 0 , wx.ALIGN_CENTER)
					else:
						GreyAttribute = 1
		...
		self.hbox9.Add(self.Status, 0 , wx.ALIGN_CENTER)
		...
		self.vbox.Add(self.hbox9, 0, wx.ALIGN_CENTER)
			...
			if ActiveMoves != None:
				while Moves < len(ActiveMoves):
					if int(Status_Pressed) == ActiveMoves[Moves]:
						Stop = Board().BoardMovement(ActivePiece, Status_Pressed)
					if Stop == 1:
