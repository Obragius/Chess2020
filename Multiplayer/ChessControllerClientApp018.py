...
if START:
	...
	def send(msg):
		...
		Confirm = Chess.AllowPacks
		if Confirm == "True":
			Packs = CONFIRMATION.encode(FORMAT)
		else:
			Packs = "False".encode(FORMAT)
		msg_length3 = len(Packs)
	...
	def ListenForBoard():
		...
		while True:
			try:
				msg_l = client.recv(HEADER).decode(FORMAT)
				msg_l = int(msg_l)
				msglist = client.recv(msg_l)
				Chess.MessageList = []
				NewList = pickle.loads(msglist)
				for Value in NewList:
					Chess.MessageList.append(Value)
				print("break")
				print(NewList)
				Chess.AllowPacks="True"
				break
			except:
				print("continue")
				continue
		...
		while Chess.ChessRun:
			try:
				...
				try:
					print('ok')
					Detected = client.recv(10).decode(FORMAT)
					print(Detected)
				except Exception:
					print(Detected)
